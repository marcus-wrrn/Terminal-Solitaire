use crate::game_objects::{Board, Card};
use ratatui::{
    buffer::Buffer,
    layout::Rect,
    style::{Color, Style},
...
impl<'a> BoardRenderer<'a> {
    ...
    fn render_stock_and_waste(&self, buf: &mut Buffer, start_y: u16) {
        let stock_x = 2;
        let waste_x = stock_x + Card::WIDTH + Self::HORIZONTAL_SPACING;

        // Stock pile
        self.render_pile_label(buf, stock_x, start_y, "Stock");
        self.board.stock.render(buf, stock_x, start_y + 1);

        // Waste pile
        self.render_pile_label(buf, waste_x, start_y, "Waste");
        self.board.waste.render(buf, waste_x, start_y + 1);
    }
    ...
    fn render_foundations(&self, buf: &mut Buffer, start_y: u16) {
        ...
        for (i, pile) in self.board.foundation.iter().enumerate() {
            let x = start_x + (i as u16) * (Card::WIDTH + Self::HORIZONTAL_SPACING);

            self.render_pile_label(buf, x, start_y, &format!("F{}", i + 1));
            pile.render(buf, x, start_y + 1);
        }
    }
...
impl<'a> Widget for BoardRenderer<'a> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        // Tableau piles (below)
        let tableau_y = stock_waste_y + Card::HEIGHT + Self::VERTICAL_SPACING + 1;
